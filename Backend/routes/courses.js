...
// @route GET api/course
// Get all courses per tutor
// Public

router.get("/courses/tutor/:id", async (req, res, next) => {
    try {
        const tutor = await User.findOne({ _id: req.params.id });
        let courses = [];
        for (courseid of tutor.coursesTeaching) {
...
router.post(
    ...
    [
        [
            check("courseid", "course is required").not().isEmpty(),
            check("tutorid", "tutor is required").not().isEmpty(),
        ],
    ],
    async (req, res, next) => {
        const errors = validationResult(req);
        ...
        try {
            ...
            console.log(tutor);
            tutor = await User.findByIdAndUpdate(
                tutorid,
                { $push: { coursesTeaching: courseid } },
                { new: true }
            );
            res.json(tutor);
        } catch (error) {
